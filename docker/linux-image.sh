...
main() {
    ...
    *)
    ...
    curl --retry 3 -sSfL 'https://www.ports.debian.org/archive_{2020,2021,2022,2023,2024,2025,2026}.key' -O
    if [ $dist == "trixie" ]; then
        curl --retry 3 -sSfL 'https://ftp-master.debian.org/keys/archive-key-13.asc' -O
        curl --retry 3 -sSfL 'https://ftp-master.debian.org/keys/archive-key-13-security.asc' -O
        curl --retry 3 -sSfL 'https://ftp-master.debian.org/keys/release-13.asc' -O
    fi

    for key in *.asc *.key; do
        mv "${key}" /etc/apt/trusted.gpg.d/
    done
