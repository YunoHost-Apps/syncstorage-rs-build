#!/usr/bin/env bash

set -euo pipefail

build_tag="$1"
publish_requested=false

if [ -z "$build_tag" ]; then
  echo "Usage: $0 <build_tag> [--publish]"
  echo "Example: $0 v2.3.6"
  exit 1
fi

if [ "${2:-}" = "--publish" ]; then
  publish_requested=true
elif [ -n "${2:-}" ]; then
  echo "Unknown argument: $2"
  echo "Usage: $0 <build_tag> [--publish]"
  exit 1
fi

script_dir="$(dirname "$(realpath "$0")")"
repo_root="$(realpath "$script_dir/..")"
diesel_dir="$repo_root/diesel"
artifact_dir="$repo_root/diesel-$build_tag"
release_tag="diesel-$build_tag"

require_container_engine() {
  if ! command -v docker >/dev/null 2>&1 && ! command -v podman >/dev/null 2>&1; then
    echo "A container engine is required by cross-rs. Install docker or podman."
    exit 1
  fi
}

prepare_diesel_checkout() {
  if [ ! -d "$diesel_dir/.git" ]; then
    git clone https://github.com/diesel-rs/diesel "$diesel_dir"
  fi

  git -C "$diesel_dir" fetch --tags origin
  git -C "$diesel_dir" checkout --detach "$build_tag"
}

publish_artifacts() {
  if ! command -v gh >/dev/null 2>&1; then
    echo "Skipping publish: gh CLI is not installed."
    return
  fi

  if ! gh auth status >/dev/null 2>&1; then
    echo "Skipping publish: gh CLI is not authenticated."
    return
  fi

  if [ ! -d "$artifact_dir" ]; then
    echo "Skipping publish: artifact directory not found: $artifact_dir"
    return
  fi

  shopt -s nullglob
  local assets=("$artifact_dir"/*.zst)
  shopt -u nullglob

  if [ "${#assets[@]}" -eq 0 ]; then
    echo "Skipping publish: no .zst artifacts were produced."
    return
  fi

  local release_title="Diesel $build_tag (pgsql)"
  local notes="[Diesel-rs](https://github.com/diesel-rs/diesel) built from tag [$build_tag](https://github.com/diesel-rs/diesel/releases/tag/$build_tag), \`postgres\` only"

  if gh release view "$release_tag" >/dev/null 2>&1; then
    gh release edit "$release_tag" --title "$release_title" --notes "$notes"
    gh release upload "$release_tag" "${assets[@]}" --clobber
  else
    gh release create "$release_tag" "${assets[@]}" --title "$release_title" --notes "$notes" --draft
  fi
}

require_container_engine
prepare_diesel_checkout

# Build diesel_cli
"$script_dir/internal/build.sh" diesel diesel_cli diesel bookworm "$build_tag" --features postgres --no-default-features
if [ "$publish_requested" = true ]; then
  publish_artifacts
else
  echo "Skipping publish: pass --publish to upload artifacts to GitHub Releases."
fi
